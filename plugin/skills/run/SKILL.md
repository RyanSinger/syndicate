...
5. **Ratchet (exploration only).** Do exactly one of: (a) add a criterion the best variant doesn't already satisfy at 5, (b) split a vague criterion into sharper ones, (c) raise the bar on an existing criterion. Pruning is allowed but does not substitute for the ratchet. Softening (lowering a bar, removing a failing criterion, splitting a bad score into two mediocre ones) is forbidden — Darwin Godel Machine 2025 on reward-hook removal. Note the ratchet action and whether it sharpens or softens in meta-notes.

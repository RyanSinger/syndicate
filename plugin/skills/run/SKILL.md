...
8. **Record what you learned.** Update `meta-notes.md`: what was tried in parallel, what worked, what didn't. Document what worked in pruned variants so future generations can recover those ideas via the `borrow` mutation operator. If a pattern recurs and is reusable, promote it to a learned agent or domain skill. Distill meta-notes when they get long.
